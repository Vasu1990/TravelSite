...
$(document).ready(function(){
	...
	(function focusOnTab(){
		...
		if(currentUrl != ''){
			$('.page-search-tabs div').removeClass('active');
			$('.search-tab-content').css('display','none');
			if(currentUrl.indexOf('hotelTab') > -1){
				$('#hotelTab').addClass('active');
				$('.search-tab-content.hotel-tab').css('display','block');
			}
			else if(currentUrl.indexOf('holidayTab') > -1){
				$('#holidayTab').addClass('active');
				$('.search-tab-content.holiday-tab').css('display','block');
			}
			else if(currentUrl.indexOf('flightTab') > -1){
				$('#flightTab').addClass('active');
				$('.search-tab-content.flight-tab').css('display','block');
			}
			else if(currentUrl.indexOf('activityTab') > -1){
				$('#activityTab').addClass('active');
				$('.search-tab-content.tour-tab').css('display','block');
			}
			else {
				$('#hotelTab').addClass('active');
				$('.search-tab-content.hotel-tab').css('display','block');
	...
	$('.badge-compare').click(function () {
		$(this).toggleClass('badge-chosen');
		$(this).closest('div.cat-list-item').toggleClass('badge-compare-selected');
		var addedToCompare = $(this).hasClass('badge-chosen');
		addedToCompare ? addToCompare.call(this) : removeFromCompare.call(this);
		$('.compare-badge').text(compareCount);
	});
	function addToCompare(){

		if(compareCount >=4){
			alert('cannot compare more that 4 items');
			$(this).removeClass('badge-chosen');
				$(this).closest('div.cat-list-item').removeClass('badge-compare-selected');
		}else{
			$(this).text('Added to Compare');
			$('#compare-dialog-add').modal('show');
			compareCount+=1;
		}
	}
	function removeFromCompare(){
		...
		if(compareCount === 0){
			...
			$('#compare-dialog-remove').modal('show');
			compareCount-=1;
	...
	$headerUp = function() {
		...
		//$('.header-logo').css('margin-top','-1px');
	...
	$('.flight-line-b b').click(function(){
		if ( $(this).is('.open') ) {
			$(this).removeClass('open');
			$(this).closest('.cat-list-item').find('.alt-details').slideUp();
		} else {
			$(this).addClass('open');
			$(this).closest('.cat-list-item').find('.alt-details').slideDown();
		}
	});
